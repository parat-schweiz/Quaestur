...
namespace Publicus
{
    ...
    public class SystemWideFileEdit : PublicusModule
    {
        public SystemWideFileEdit()
        {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (status.ObjectNotNull(systemWideFile))
                    {
                        ...
                        if (status.IsSuccess)
                        {
                            ...
                            Notice("{0} changed system wide file {1}", CurrentSession.User.UserName, systemWideFile);
                        }
                    }
                }
            ...
            Post("/systemwidefile/add/new", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    {
                        Database.Save(systemWideFile);
                        Notice("{0} added system wide file {1}", CurrentSession.User.UserName, systemWideFile);
                    }
                }
                ...
                if (HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (systemWideFile != null)
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            ...
                            transaction.Commit();
                            Notice("{0} deleted system wide file {1}", CurrentSession.User.UserName, systemWideFile);
                        }
                    }
                }
                return string.Empty;
            ...
            Get("/systemwidefile/download/{id}", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (status.ObjectNotNull(systemWideFile))
                    {
                        ...
                        Notice("{0} downloaded system wide file {1}", CurrentSession.User.UserName, systemWideFile);
