...
namespace Census
{
    ...
    public class OrganizationListViewModel
    {
        ...
        public OrganizationListViewModel(Translator translator, Session session, IDatabase database)
        {
            ...
            PhraseDeleteConfirmationInfo = translator.Get("Organization.List.Delete.Confirm.Info", "Delete organization confirmation info", "This will also delete all subscriptions, groups, roles, permissions and mailings under that organization.").EscapeHtml();
    ...
    public class OrganizationEdit : CensusModule
    {
        public OrganizationEdit()
        {
                ...
                if (status.ObjectNotNull(organization))
                {
                    if (status.HasAccess(organization, PartAccess.Structure, AccessRight.Write))
                    {
                        ...
                        if (status.IsSuccess)
                        {
                            ...
                            Notice("{0} changed organization {1}", CurrentSession.User.UserName.Value, organization);
                        }
                    }
                }
            ...
            Post("/organization/add/new", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.Structure, AccessRight.Write))
                {
                    ...
                    {
                        Database.Save(organization);
                        Notice("{0} added organization {1}", CurrentSession.User.UserName.Value, organization);
                    }
                }
                ...
                if (status.ObjectNotNull(organization))
                {
                    if (status.HasAccess(organization, PartAccess.Structure, AccessRight.Write))
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            ...
                            transaction.Commit();
                            Notice("{0} deleted organization {1}", CurrentSession.User.UserName.Value, organization);
                        }
                    }
                }
