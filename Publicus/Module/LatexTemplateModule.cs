...
namespace Publicus
{
    public class LatexTemplateEditViewModel : MasterViewModel
    {
            ...
            : this(translator, session)
        {
            ...
            AssignmentTypes = new List<NamedIntViewModel>();
            AssignmentTypes.Add(new NamedIntViewModel(translator, TemplateAssignmentType.Petition, TemplateAssignmentType.Petition == latexTemplate.AssignmentType.Value));
            Organizations = new List<NamedIdViewModel>(database
                .Query<Feed>()
                .Select(o => new NamedIdViewModel(translator, o, latexTemplate.Feed.Value == o))
                .OrderBy(o => o.Name));
        }
    }
    ...
    public class LatexTemplateListItemViewModel
    {
        ...
        public LatexTemplateListItemViewModel(IDatabase database, Session session, Translator translator, LatexTemplate latexTemplate)
        {
            ...
            Editable = session.HasAccess(latexTemplate.Feed.Value, latexTemplate.AssignmentType.Value.AccessPart(), AccessRight.Write) ? "editable" : string.Empty;
    ...
    public class LatexTemplateEdit : PublicusModule
    {
        public LatexTemplateEdit()
        {
            ...
            Get("/latextemplate/edit/{id}", parameters =>
            {
                ...
                {
                    if (HasAccess(latexTemplate.Feed.Value, 
            ...
            Post("/latextemplate/edit/{id}", parameters =>
            {
                ...
                if (status.ObjectNotNull(latexTemplate))
                {
                                  ...
                                  AccessRight.Write))
                    {
                        ...
                        status.AssignObjectIdString("Feed", latexTemplate.Feed, model.Organization);
                        ...
                        if (usedInAssignment != null &&
                            usedInAssignment.GetFeed(Database) != latexTemplate.Feed.Value)
                        {
                        ...
                        if (status.IsSuccess)
                        {
                            if (status.HasAccess(latexTemplate.Feed.Value,
                                                 latexTemplate.AssignmentType.Value.AccessPart(),
                                                 AccessRight.Write))
                            {
                                Database.Save(latexTemplate);
                                Notice("{0} changed latex template {1}", CurrentSession.User.UserName.Value, latexTemplate);
                            }
                        }
                    }
                    else
            ...
            Post("/latextemplate/add/new", parameters =>
            {
                ...
                if (status.IsSuccess)
                {
                    ...
                    {
                        Database.Save(latexTemplate);
                        Notice("{0} added latex template {1}", CurrentSession.User.UserName.Value, latexTemplate);
                    }
                }
                ...
                if (latexTemplate != null)
                {
                                  ...
                                  AccessRight.Write))
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            ...
                            transaction.Commit();
                            Notice("{0} deleted latex template {1}", CurrentSession.User.UserName.Value, latexTemplate.GetText(Translator));
                        }
                    }
                }
            ...
            Get("/latextemplate/copy/{id}", parameters =>
            {
                ...
                if (latexTemplate != null)
                {
                                  ...
                                  AccessRight.Write))
                    {
                        ...
                        newTemplate.Feed.Value = latexTemplate.Feed.Value;
                        ...
                        Notice("{0} added latex template {1}", CurrentSession.User.UserName.Value, newTemplate.GetText(Translator));
                    }
                }
