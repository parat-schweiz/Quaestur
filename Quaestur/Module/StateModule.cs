        ...
        public StateEdit()
        {
            ...
            Post("/state/edit/{id}", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (status.ObjectNotNull(state))
                    {
                        ...
                        {
                            Database.Save(state);
                            Notice("{0} changed state {1}", CurrentSession.User.ShortHand, state);
                        }
                    }
                }
            ...
            Post("/state/add/new", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    {
                        Database.Save(state);
                        Notice("{0} added state {1}", CurrentSession.User.ShortHand, state);
                    }
                }
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (status.ObjectNotNull(state))
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            ...
                            transaction.Commit();
                            Notice("{0} deleted state {1}", CurrentSession.User.ShortHand, state);
                        }
                    }
                }
