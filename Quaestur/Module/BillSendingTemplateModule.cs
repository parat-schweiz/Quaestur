                    ...
                    if (status.HasAccess(billSendingTemplate.MembershipType.Value.Organization.Value, PartAccess.Structure, AccessRight.Write))
                    {
                        ...
                        if (status.IsSuccess)
                        {
                            using (var transaction = Database.BeginTransaction())
                            {
                                ...
                                status.UpdateLatexTemplates(Database, billSendingTemplate.BillSendingLetters, model.BillSendingLetterTemplates);
                                status.UpdateMailTemplates(Database, billSendingTemplate.BillSendingMails, model.BillSendingMailTemplates);
                                transaction.Commit();
