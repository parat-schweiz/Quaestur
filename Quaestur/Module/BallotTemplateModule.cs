    ...
    public class BallotTemplateModule : QuaesturModule
    {
        public BallotTemplateModule()
        {
                    ...
                    if (status.HasAccess(ballotTemplate.Organizer.Value.Organization.Value, PartAccess.Ballot, AccessRight.Write))
                    {
                            ...
                            status.HasAccess(ballotTemplate.Organizer.Value.Organization.Value, PartAccess.Ballot, AccessRight.Write))
                        {
                            using (var transaction = Database.BeginTransaction())
                            {
                                ...
                                status.UpdateMailTemplates(Database, ballotTemplate.AnnouncementMails, model.AnnouncementMailTemplates);
                                status.UpdateMailTemplates(Database, ballotTemplate.InvitationMails, model.InvitationMailTemplates);
                                status.UpdateLatexTemplates(Database, ballotTemplate.BallotPapers, model.BallotPaperTemplates);
            ...
            Get("/ballottemplate/test/{id}", parameters =>
            {
                ...
                {
                    result.MessageType = "warning";
                    result.MessageText = Translate("BallotTemplate.Test.Failed.NotFound", "Object not found during test create ballot", "Object not found");
                    result.IsSuccess = false;
                }
