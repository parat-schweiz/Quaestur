...
namespace Quaestur
{
    ...
    public class MailTemplateListItemViewModel
    {
        ...
        public MailTemplateListItemViewModel(IDatabase database, Session session, Translator translator, MailTemplate mailTemplate)
        {
            ...
            Editable = session.HasAccess(mailTemplate.Organization.Value, mailTemplate.AssignmentType.Value.Value.AccessPart(), AccessRight.Write) ? "editable" : string.Empty;
    ...
    public class MailTemplateListViewModel
    {
        ...
        public MailTemplateListViewModel(IDatabase database, Translator translator, Session session)
        {
            ...
            List = new List<MailTemplateListItemViewModel>(
                ...
                .Where(t => session.HasAccess(t.Organization.Value, t.AssignmentType.Value.Value.AccessPart(), AccessRight.Read))
    ...
    public class MailTemplateEdit : QuaesturModule
    {
        public MailTemplateEdit()
        {
            ...
            Get("/mailtemplate/edit/{id}", parameters =>
            {
                ...
                {
                    if (HasAccess(mailTemplate.Organization.Value,
                                  mailTemplate.AssignmentType.Value.Value.AccessPart(),
                                  AccessRight.Write))
                    {
            ...
            Post("/mailtemplate/edit/{id}", parameters =>
            {
                ...
                if (status.ObjectNotNull(mailTemplate))
                {
                                  ...
                                  AccessRight.Write))
                    {
                        ...
                        mailTemplate.HtmlText.Value = worker.PlainText;
                        ...
                        if (usedInAssignment != null &&
                            usedInAssignment.GetOrganization(Database) != mailTemplate.Organization.Value)
                        {
                        ...
                        if (status.IsSuccess)
                        {
                            if (status.HasAccess(mailTemplate.Organization.Value,
                                                 mailTemplate.AssignmentType.Value.Value.AccessPart(),
                                                 AccessRight.Write))
                            {
                                Database.Save(mailTemplate);
                                Notice("{0} changed mail template {1}", CurrentSession.User.ShortHand, mailTemplate);
                            }
                        }
                    }
                    else
            ...
            Post("/mailtemplate/add/new", parameters =>
            {
                ...
                if (status.IsSuccess)
                {
                    ...
                    {
                        Database.Save(mailTemplate);
                        Notice("{0} added mail template {1}", CurrentSession.User.ShortHand, mailTemplate);
                    }
                }
                ...
                if (mailTemplate != null)
                {
                                  ...
                                  AccessRight.Write))
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            ...
                            transaction.Commit();
                            Notice("{0} deleted mail template {1}", CurrentSession.User.ShortHand, mailTemplate.GetText(Translator));
                        }
                    }
                }
