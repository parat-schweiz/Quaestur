...
namespace Quaestur
{
    ...
    public class RoleListItemViewModel
    {
        ...
        public RoleListItemViewModel(Translator translator, IDatabase database, Session session, Role role)
        {
            ...
            Occupants = string.Join("<br/>", database
                ...
                .Select(ra => ra.Person.Value.ShortHand)
                .OrderBy(p => p));
            if (string.IsNullOrEmpty(Occupants))
                Occupants = translator.Get("Role.List.Occupants.None", "No occupants in role list", "None");
            Editable =
                session.HasAccess(role.Group.Value.Organization.Value, PartAccess.Structure, AccessRight.Write) ?
    ...
    public class RoleListViewModel
    {
        ...
        public string PhraseHeaderOrganizationGroup;
        ...
        {
            PhraseHeaderOrganizationGroup = 
                translator.Get("Role.List.Header.OrganizationGroup", "Header with organization and group in the role list", "{0} of {1}",
                group.Name.Value[translator.Language],
                group.Organization.Value.Name.Value[translator.Language]).EscapeHtml();
            ...
            PhraseDeleteConfirmationInfo = translator.Get("Role.List.Delete.Confirm.Info", "Delete role confirmation info", "This will also delete all permission of that role and remove all assignments of that role to any person.").EscapeHtml();
            Id = group.Id.Value.ToString();
            ParentId = group.Organization.Value.Id.Value.ToString();
            ...
            Post("/role/edit/{id}", parameters =>
            {
                ...
                if (status.ObjectNotNull(role))
                {
                    if (status.HasAccess(role.Group.Value, PartAccess.Structure, AccessRight.Write))
                    {
                        ...
                        {
                            Database.Save(role);
                            Notice("{0} changed role {1}", CurrentSession.User.ShortHand, role);
                        }
                    }
                }
            ...
            Post("/role/add/{id}", parameters =>
            {
                ...
                if (status.ObjectNotNull(group))
                {
                    if (status.HasAccess(group, PartAccess.Structure, AccessRight.Write))
                    {
                        ...
                        {
                            Database.Save(role);
                            Notice("{0} added role {1}", CurrentSession.User.ShortHand, role);
                        }
                    }
                }
                ...
                if (status.ObjectNotNull(role))
                {
                    if (status.HasAccess(role.Group.Value, PartAccess.Structure, AccessRight.Write))
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            ...
                            transaction.Commit();
                            Notice("{0} deleted role {1}", CurrentSession.User.ShortHand, role);
                        }
                    }
                }
