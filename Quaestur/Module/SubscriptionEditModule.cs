...
namespace Quaestur
{
    public class SubscriptionEditViewModel : DialogViewModel
    {
            ...
            : this(translator)
        {
            ...
            SenderGroups = database
                ...
                .OrderBy(t => t.Name)
                .ToList();
            SubscribePrePages = database
               .Query<MailTemplate>()
               .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
               .Select(t => new NamedIdViewModel(translator, t, false))
               .OrderBy(t => t.Name)
               .ToList();
            SubscribePostPages = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            SubscribeMails = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            UnsubscribePrePages = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            UnsubscribePostPages = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            JoinPrePages = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            JoinPostPages = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            JoinConfirmMails = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
                .ToList();
            ConfirmMailPages = database
                .Query<MailTemplate>()
                .Where(t => t.AssignmentType.Value == TemplateAssignmentType.Subscription)
                .Select(t => new NamedIdViewModel(translator, t, false))
                .OrderBy(t => t.Name)
    ...
    public class SubscriptionEditModule : QuaesturModule
    {
        ...
        {
            status.UpdateMailTemplates(Database, subscription.SubscribePrePages, model.SubscribePrePageTemplates);
            status.UpdateMailTemplates(Database, subscription.SubscribePostPages, model.SubscribePostPageTemplates);
            status.UpdateMailTemplates(Database, subscription.SubscribeMails, model.SubscribeMailTemplates);
            status.UpdateMailTemplates(Database, subscription.UnsubscribePrePages, model.UnsubscribePrePageTemplates);
            status.UpdateMailTemplates(Database, subscription.UnsubscribePostPages, model.UnsubscribePostPageTemplates);
            status.UpdateMailTemplates(Database, subscription.JoinPrePages, model.JoinPrePageTemplates);
            status.UpdateMailTemplates(Database, subscription.JoinPostPages, model.JoinPostPageTemplates);
            status.UpdateMailTemplates(Database, subscription.JoinConfirmMails, model.JoinConfirmMailTemplates);
            status.UpdateMailTemplates(Database, subscription.ConfirmMailPages, model.ConfirmMailPageTemplates);
        }
