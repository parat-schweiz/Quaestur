...
namespace Quaestur
{
    public class LatexTemplateEditViewModel : MasterViewModel
    {
            ...
            : this(translator, session)
        {
            ...
            AssignmentTypes = new List<NamedIntViewModel>();
            AssignmentTypes.Add(new NamedIntViewModel(translator, TemplateAssignmentType.BallotTemplate, TemplateAssignmentType.BallotTemplate == latexTemplate.AssignmentType.Value));
    ...
    public class LatexTemplateListItemViewModel
    {
        ...
        public LatexTemplateListItemViewModel(IDatabase database, Session session, Translator translator, LatexTemplate latexTemplate)
        {
            ...
            Editable = session.HasAccess(latexTemplate.Organization.Value, latexTemplate.AssignmentType.Value.Value.AccessPart(), AccessRight.Write) ? "editable" : string.Empty;
            ...
            Get("/latextemplate/edit/{id}", parameters =>
            {
                ...
                if (latexTemplate != null)
                {
                    if (HasAccess(latexTemplate.Organization.Value, 
                                  latexTemplate.AssignmentType.Value.Value.AccessPart(), 
                                  AccessRight.Write))
                    {
