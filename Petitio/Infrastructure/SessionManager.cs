    ...
    public class Session : ClaimsPrincipal
    {
        ...
        public bool HasContactNewAccess()
        {
            return RoleAssignments
                .Select(ra => ra.Role.Value.Group.Value.Queue.Value)
        ...
        public bool HasAnyQueueAccess(PartAccess partAccess, AccessRight right)
        {
            foreach (var rolePermission in _access
                .Where(rp => rp.Permission.Part.Value == partAccess)
                .Where(rp => rp.Permission.Right.Value >= right))
        ...
        private bool HasThisAccess(RolePermission rp)
        {
            switch (rp.Permission.Subject.Value)
            {
                ...
                case SubjectAccess.Queue:
                    return HasAccess(rp.Role.Group.Value.Queue.Value, rp.Permission.Part.Value, rp.Permission.Right.Value);
               case SubjectAccess.SubQueue:
                    return HasAccess(rp.Role.Group.Value.Queue.Value, rp.Permission.Part.Value, rp.Permission.Right.Value) &&
                        rp.Role.Group.Value.Queue.Value.Children.All(c =>
        ...
        public bool HasAccess(Contact contact, PartAccess partAccess, AccessRight right)
        {
            ...
            {
                if (HasAccess(subscription.Queue.Value, partAccess, right))
                {
                    return true; 
                } 
            }
        ...
        public bool HasAccess(Queue queue, PartAccess partAccess, AccessRight right)
        {
            foreach (var rolePermission in _access
                .Where(rp => rp.Permission.Part.Value == partAccess)
                .Where(rp => rp.Permission.Right.Value >= right))
            {
                switch (rolePermission.Permission.Subject.Value)
                {
                    case SubjectAccess.SystemWide:
                        return true;
                    case SubjectAccess.Queue:
                        if (rolePermission.Role.Group.Value.Queue.Value == queue)
                        {
                            return true; 
                        }
                        break;
                    case SubjectAccess.SubQueue:
                        ...
                        else if (rolePermission.Role.Group.Value.Queue.Value
                            .Subordinates.Contains(queue))
                        {
                            return true; 
                        }
                        break;
                ...
                .Where(rp => rp.Permission.Right.Value >= right))
            {
                switch (rolePermission.Permission.Subject.Value)
                {
                    case SubjectAccess.SystemWide:
                        return true;
                    case SubjectAccess.Queue:
                        if (rolePermission.Role.Group.Value.Queue.Value
