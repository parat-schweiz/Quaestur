...
namespace Petitio
{
    ...
    public class RoleAssignmentListViewModel
    {
        ...
        public RoleAssignmentListViewModel(Translator translator, IDatabase database, Session session, Role role)
        {
            ...
            PhraseHeaderRoleGroupQueue =
                translator.Get("RoleAssignment.List.Header.RoleGroupQueue", "Header part 'Queue' in the permission list", "{0} in {1} of {2}",
    ...
    public class RoleAssignmentModule : PetitioModule
    {
        public static bool IsAssingmentPermitted(Session session, Role role)
        {
            ...
            else if (role.Permissions.Any(p => p.Subject.Value == SubjectAccess.SubQueue))
            {
                return session.HasAccess(role.Group.Value.Queue.Value, PartAccess.RoleAssignments, AccessRight.Write);
            }
            else if (role.Permissions.Any(p => p.Subject.Value == SubjectAccess.Queue))
            {
                return session.HasAccess(role.Group.Value.Queue.Value, PartAccess.RoleAssignments, AccessRight.Write);
            }
            else
