...
namespace Petitio
{
    public class ExportEditViewModel : DialogViewModel
    {
        ...
        public string PhraseFieldSelectQueue;
                   ...
                   "exportEditDialog")
        {
            ...
            PhraseFieldSelectQueue = translator.Get("Export.Edit.Field.SelectQueue", "Field 'SelectQueue' in the edit export dialog", "Select by Queue").EscapeHtml();
            ...
            : this(translator)
        {
            ...
            Id = "new";
            Name = string.Empty;
            SelectQueue = string.Empty;
            SelectTag = string.Empty;
            SelectLanguage = string.Empty;
            Queues = new List<NamedIdViewModel>(db
                .Query<Queue>()
            ...
            if (session.HasSystemWideAccess(PartAccess.Demography, AccessRight.Read))
                Queues.Add(new NamedIdViewModel(translator.Get("Export.Edit.Field.SelectQueue.None", "No selection in the select queue field of the edit export page", "None"), false, true));
            ...
            : this(translator)
        {
            ...
            Queues = new List<NamedIdViewModel>(db
                .Query<Queue>()
                .Where(o => session.HasAccess(o, PartAccess.Demography, AccessRight.Read))
                .Select(o => new NamedIdViewModel(translator, o, o == export.SelectQueue.Value))
                .OrderBy(o => o.Name));
            if (session.HasSystemWideAccess(PartAccess.Demography, AccessRight.Read))
                Queues.Add(new NamedIdViewModel(translator.Get("Export.Edit.Field.SelectQueue.None", "No selection in the select queue field of the edit export page", "None"), false, export.SelectQueue.Value == null));
            Tags = new List<NamedIdViewModel>(db
                .Query<Tag>()
    ...
    public class ExportEdit : PetitioModule
    {
        public ExportEdit()
        {
            ...
            {
                if (HasAnyQueueAccess(PartAccess.Demography, AccessRight.Read))
                {
            ...
            Post("/export/edit/{id}", parameters =>
            {
                ...
                if (status.ObjectNotNull(export))
                {
                    ...
                    status.AssignObjectIdString("SelectQueue", export.SelectQueue, model.SelectQueue);
        ...
        private IEnumerable<Contact> Query(Export export)
        {
            return Database.Query<Contact>()
                .Where(p => export.SelectQueue.Value == null || p.ActiveSubscriptions.Any(m => m.Queue.Value == export.SelectQueue.Value))
        ...
        private bool IsPermittedExport(Export export)
        {
            ...
            if (export.SelectQueue.Value == null)
            {
                return columns
            ...
            else
            {
                return columns
                    .ComputeRequiredAccess(export.ExportColumns.Value)
                    .All(pa => HasAccess(export.SelectQueue.Value, pa, AccessRight.Read));
            }
        }
    }
