...
namespace Petitio
{
    public class PermissionEditViewModel : DialogViewModel
    {
            ...
            : this(translator)
        {
            ...
            Subjects.Add(new NamedIntViewModel(translator, SubjectAccess.Queue, false));
            Subjects.Add(new NamedIntViewModel(translator, SubjectAccess.SubQueue, false));
            ...
            : this(translator)
        {
            ...
            Subjects.Add(new NamedIntViewModel(translator, SubjectAccess.Queue, permission.Subject.Value == SubjectAccess.Queue));
            Subjects.Add(new NamedIntViewModel(translator, SubjectAccess.SubQueue, permission.Subject.Value == SubjectAccess.SubQueue));
    ...
    public class PermissionListViewModel
    {
        ...
        public string PhraseHeaderRoleGroupQueue;
        ...
        public PermissionListViewModel(Translator translator, IDatabase database, Session session, Role role)
        {
            ...
            PhraseHeaderRoleGroupQueue =
                translator.Get("Permission.List.Header.RoleGroupQueue", "Header part 'Queue' in the permission list", "{0} in {1} of {2}",
                role.Name.Value[translator.Language],
                role.Group.Value.Name.Value[translator.Language],
                role.Group.Value.Queue.Value.Name.Value[translator.Language]).EscapeHtml();
    ...
    public class PermissionEdit : PetitioModule
    {
        private bool IsPermissionPermitted(Permission permission)
        {
            switch (permission.Subject.Value)
            {
                ...
                case SubjectAccess.Queue:
                case SubjectAccess.SubQueue:
                    return HasAccess(permission.Role.Value.Group.Value.Queue.Value, PartAccess.Structure, AccessRight.Write);
