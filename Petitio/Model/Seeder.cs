...
namespace Petitio
{
    public class Seeder
    {
            ...
            if (role == null)
            {
                ...
                switch (profile)
                {
                    ...
                    case RoleProfile.BoardAdmin:
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.CustomDefinitions, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Structure, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Contact, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Subscription, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.RoleAssignments, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.TagAssignments, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Demography, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Documents, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Mailings, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Anonymous, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Journal, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Crypto, AccessRight.Write);
                        break;
                    case RoleProfile.BoardMember:
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.CustomDefinitions, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Structure, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Subscription, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.RoleAssignments, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Contact, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.TagAssignments, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Demography, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Documents, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Mailings, AccessRight.Write);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Anonymous, AccessRight.Read);
                        AddPermission(role, SubjectAccess.SubQueue, PartAccess.Journal, AccessRight.Read);
                        break;
        ...
        private Group GetGroup(Queue queue, string name)
        {
            var group = queue.Groups.FirstOrDefault(g => g.Name.Value.AnyValue == name);

            if (group == null)
            {
                group = new Group(Guid.NewGuid());
                group.Name.Value[Language.German] = name;
                group.MailName.Value[Language.German] = queue.Name.Value.AnyValue + " " + name;
                group.MailAddress.Value[Language.German] = 
                    name.ToLowerInvariant().Replace(" ", "-") + "@" +
                    queue.Name.Value.AnyValue.ToLowerInvariant().Replace(" ", "-") + ".ch";
        ...
        private Contact CreateContact(int number)
        {
            ...
            subscription.Queue.Value = GetQueue("Piratenpartei Zentralschweiz", null);
            ...
            switch (_rnd.Next(4))
            {
                case 0:
                    ...
                    subscription2.Queue.Value = GetQueue("Piratenpartei Zug", "Piratenpartei Zentralschweiz");
                ...
                case 1:
                    ...
                    subscription3.Queue.Value = GetQueue("Piratenpartei Luzern", "Piratenpartei Zentralschweiz");
        ...
        private Queue GetQueue(string name, string parent)
        {
            var queue = _db.Query<Queue>().FirstOrDefault(o => o.Name.Value.AnyValue == name);

            if (queue == null)
            {
                queue = new Queue(Guid.NewGuid());
                queue.Name.Value[Language.German] = name;

                if (parent != null && parent.Length > 0)
                {
                    queue.Parent.Value = GetQueue(parent, null); 
                }
