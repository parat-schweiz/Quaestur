...
namespace Hospes
{
    ...
    public class TagListViewModel
    {
        ...
        public TagListViewModel(Translator translator, IDatabase database)
        {
            ...
            PhraseDeleteConfirmationInfo = translator.Get("Tag.List.Delete.Confirm.Info", "Delete tag confirmation info", "This will remove that tag from all persons.").EscapeHtml();
    ...
    public class TagEdit : QuaesturModule
    {
        public TagEdit()
        {
            ...
            Post("/tag/edit/{id}", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (status.ObjectNotNull(tag))
                    {
                        ...
                        {
                            Database.Save(tag);
                            Notice("{0} changed tag {1}", CurrentSession.User.ShortHand, tag);
                        }
                    }
                }
            ...
            Post("/tag/add/new", parameters =>
            {
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    {
                        Database.Save(tag);
                        Notice("{0} added tag {1}", CurrentSession.User.ShortHand, tag);
                    }
                }
                ...
                if (status.HasSystemWideAccess(PartAccess.CustomDefinitions, AccessRight.Write))
                {
                    ...
                    if (status.ObjectNotNull(tag))
                    {
                        using (var transaction = Database.BeginTransaction())
                        {
                            tag.Delete(Database);
                            transaction.Commit();
                            Notice("{0} deleted tag {1}", CurrentSession.User.ShortHand, tag);
                        }
                    }
                }
