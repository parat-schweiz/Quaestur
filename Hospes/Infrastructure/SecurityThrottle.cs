        ...
        private void FailInternal(string username, bool secondFactor, bool global)
        {
            ...
            if (global)
            {
                switch (timeCount)
                {
                    ...
                    case 5000:
                    case 25000:
                        Global.Log.Warning("Hospes recorded {1} total wrong {0}.", secondFactor ? "2fas" : "logins", timeCount);
                        Global.Mail.SendAdmin("Wrong login alert", string.Format("Hospes recorded {1} total wrong {0}.", secondFactor ? "2fas" : "logins", timeCount));
                        break;
                }
            }
            else
            {
                switch (timeCount)
                {
                    ...
                    case 500:
                    case 2500:
                        Global.Log.Warning("Hospes recorded {1} wrong {0} for account {2}.", secondFactor ? "2fas" : "logins", timeCount, username);
                        Global.Mail.SendAdmin("Wrong login alert", string.Format("Hospes recorded {1} wrong {0} for account {2}.", secondFactor ? "2fas" : "logins", timeCount, username));
                        break;
                }
            }
        }
